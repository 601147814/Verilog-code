	...
	///////////////////////////////////////////////////////////////////////
	//	Module		: spi_3_wire
	//	Description	: A parameterized SPI 3-wire interface controller supporting both read and write operations
	...
	always@( posedge clk )begin
		if( busy == 1'b1 && rd_flag == 1'b1 )begin
			if((cnt == CLK_DIV/2 - 1'b1) && (cnt_bit > ADDR_WIDTH) && (cnt_bit < ADDR_WIDTH + DATA_WIDTH + 8'd1))begin
				drd_reg <= {drd_reg[0+:DATA_WIDTH-1],SDIO};			
			end
			else;
		end
		else ;
	...
	always@( posedge clk )begin
		if( busy == 1'b1 && rd_flag == 1'b1 )begin
			if( (cnt_bit == ADDR_WIDTH + DATA_WIDTH ) && ( cnt == CLK_DIV - 1'b1) )begin
				rd_flag_inout <= 1'b0;	
			end
			else if((cnt == CLK_DIV - 1'b1) && (cnt_bit == ADDR_WIDTH))begin
				rd_flag_inout <= 1'b1;	
			end
			else;
		end
	...
	assign SDIO = ( rd_flag_inout == 1'b1 ) ? 1'hz : data_reg[ADDR_WIDTH+DATA_WIDTH-1];
	assign CS_N	= cs_n_o;
	assign SCLK = sclk_o;
