...
#           v1 - test
#           v2 - add tcc match
#           v3 - add sub function that ram wrapper need to be split
# Input     :
#   
...
#   crl ts12n0c41p11sacrl128sa05p1 spsram    RF   NHS  
#   del ts12n0c41p11sadel02msa05p1 spsram    SRAM NHS
#   srl ts12n0c41p11sasrl128sa05p1 spsram    RF   HS
#   ssl ts12n0c41p11sassl01msa05p1 spsram    SRAM HS
#   cul ts12n0c42p11sacul128sa05p2 dprf_1clk RF
#   drl ts12n0c42p11sadrl128sa06p2 dprf_2clk RF
#   dsl ts12n0c42p22sadsl01msa05p1 dspsram   SRAM
#   dgl ts12n0c41p10asdg101msa04   rom

    my $chip            = $ENV{chip} ;
    my $show_log_path   = "$chip/top/test/test003/log/verilog.log" ;
    my $csv_lib_path    = '/proj/allproj/Memories_compiler/virage_12ffc_20190618/csv_lib' ;
    my $ram_lib_path    = '/proj/allproj/Memories_compiler/virage_12ffc_20190618/ram_lib' ;
    my $ram_model_file  = "$chip/ram/rtl/ram_model.f";
    my $tcc_corner      = 'ssgnp_ccwt0p72vn40c' ;
    my $power_corner    = 'tt0p8v85c' ;
    my $process         = "s0c4";
    ...
    my $compiler_range  = "/proj/allproj/Memories_compiler/virage_12ffc_20190618/virage_12nm_range_table/";
    my $compiler_name   = "";
    my $memory_name     = "";
    my $compiler_head   = "";
    my $compiler_type   = "";
    ...
    if($work_mode eq "list"){
        ...
        for($i=0;$i<$len;$i++){
            ...
            # get nw nb port(1p|2p) (cul|drl|del|crl..) pg_enable(0|1)
    ...
    sub get_wrapper_list(){
        ...
        if($ARGV[2] =~ /((\d)\.(\d+)|(\d+))/){$tcc_goal = $ARGV[2]}
        else{$tcc_goal = 5}
        print"tcc: $tcc_goal\n";
    ...
    sub mem_match(){ 
        ...
        if($wrapper_int=~/dprf_2clk_(\d+)x(\d+).*_wrapper/){
            ...
            $compiler_type = "drl";
            $compiler_name = "ts12n0c42p11sadrl128sa06p2";
            $nw = $1;$nb = $2;if($nb<4){$nb=4}
            $port = "2p";
        }elsif($wrapper_int=~/dprf_1clk_(\d+)x(\d+).*_wrapper/){
            $nw = $1; $nb = $2;$port="2p";
            $compiler_head = "sa"."cul"."$process"."s";
            $compiler_type = "cul";
            $compiler_name = "ts12n0c42p11sacul128sa05p2";
            ...
            if($prefix =~ /(a55_|a53_|a35_)/){
                if($nw <= 2048){$compiler_head = "sa"."srl"."$process"."l";$compiler_name = "ts12n0c41p11sasrl128sa05p1";$compiler_type = "srl";}
                else{           $compiler_head = "sa"."ssl"."$process"."l";$compiler_name = "ts12n0c41p11sassl01msa05p1";$compiler_type = "ssl";}
            }else{                
                if($nw <= 512 ){$compiler_head = "sa"."crl"."$process"."s";$compiler_name = "ts12n0c41p11sacrl128sa05p1";$compiler_type = "crl";}
                else{           $compiler_head = "sa"."del"."$process"."s";$compiler_name = "ts12n0c41p11sadel02msa05p1";$compiler_type = "del";}
            }
        }elsif($wrapper_int=~/dpsram_(\d+)x(\d+).*_wrapper/){
            $nw = $1;
            $nb = $2;
            $port = "2p";
            $compiler_head = "sa"."dsl"."$process"."s";
            $compiler_type = "dsl";
            $compiler_name = "ts12n0c42p22sadsl01msa05p1";
        }else{
